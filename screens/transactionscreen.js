...
import { Text, TouchableOpacity, View, StyleSheet, TextInput, Image, KeyboardAvoidingView, ToastAndroid } from "react-native";
...
checkStudentEligibilityForBookIssue=async()=>{
  ...
  if (studentRef.docs.lenght==0) {
    ...
    alert("student is not in a database")
  }
  else{
...
transactionRef.docs.map(doc=>{
  ...
  if (lastBookTransaction.studentId===this.state.scannedStudentId) {
    ...
    isStudentEligible=false
    alert("book was not issued by the student")
...
return isStudentEligible
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      alert("book is not is the database")
      this.setState({
        scannedStudentId:'',
        scannedBookId:''
      })
